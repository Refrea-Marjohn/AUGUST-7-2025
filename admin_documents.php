...
if ($filter_from && $filter_to) {
    $where_conditions[] = "DATE(upload_date) >= '" . $conn->real_escape_string($filter_from) . "' AND DATE(upload_date) <= '" . $conn->real_escape_string($filter_to) . "'";
} elseif ($filter_from) {
    $where_conditions[] = "DATE(upload_date) = '" . $conn->real_escape_string($filter_from) . "'";
} elseif ($filter_to) {
    $where_conditions[] = "DATE(upload_date) <= '" . $conn->real_escape_string($filter_to) . "'";
}
...
$result = $conn->query($attorney_query);
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $all_documents[] = $row;
    }
...
if (!empty($user_ids)) {
    ...
    $user_result = $conn->query("SELECT id, name FROM user_form WHERE id IN ($user_ids_str)");
    if ($user_result) {
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if ($source === 'admin') {
        ...
        $res = $conn->query("SELECT file_path, file_name, case_id, uploaded_by FROM attorney_documents WHERE id=$id");
        if ($res && $row = $res->fetch_assoc()) {
            @unlink($row['file_path']);
            $user_name = $_SESSION['admin_name'] ?? 'Admin';
        ...
        $res = $conn->query("SELECT file_path, file_name, form_number, uploaded_by FROM employee_documents WHERE id=$id");
        if ($res && $row = $res->fetch_assoc()) {
            @unlink($row['file_path']);
            $user_name = $_SESSION['admin_name'] ?? 'Admin';
...
<body>
    ...
    <div class="main-content">
        ...
        <?php
        $admin_count = $conn->query("SELECT COUNT(*) as count FROM admin_documents")->fetch_assoc()['count'];
        $attorney_count = $conn->query("SELECT COUNT(*) as count FROM attorney_documents")->fetch_assoc()['count'];
        $employee_count = $conn->query("SELECT COUNT(*) as count FROM employee_documents")->fetch_assoc()['count'];
